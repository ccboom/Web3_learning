...
> 从 **O（无穷远点）** 开始出发，每一次都跨出固定的距离 **G**，不停地跑。

**n 的定义**：这个运动员跑了 **n 步**之后，正好跑完一圈回到起跑点 **O**。

这就是公式 $n \times G = O$ 的含义。
...
- $k$ 是一个随机整数，这个整数**不能无限大**
- 如果选的是 $n + 1$，那就相当于只跑了一步
- 所以 **BTC 私钥必须在 $1$ 到 $n-1$ 之间**
